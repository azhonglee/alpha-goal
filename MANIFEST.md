...
|---|---:|---|
| `scripts/install.sh` | Yes | Installs the three public skills as direct symlinks, cleans same-repo old skill links, syncs optional user templates, installs or updates the compact recovery hook, and validates install targets. |
...
`scripts/install.sh` manages one user-level `SessionStart` / `^compact$` hook in `${CODEX_HOME:-$HOME/.codex}/hooks.json`, marked by `codex-alpha-goal-compact-recovery:v1`. The hook prints a static compact recovery policy that asks Codex to re-check `alpha-goal`, `control-loop`, and `evidence-verify` after compaction and reread the applicable `SKILL.md`.
