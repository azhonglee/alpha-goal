...
|---|---:|---|
| `scripts/install.sh` | Yes | Installs the three public skills as direct symlinks under `$HOME/.agents/skills`, presents a color+Unicode arrow-key target menu for interactive no-target runs, prints grouped install/uninstall summaries, adopts same Git common-dir worktree skill symlinks, cleans same-repo old skill links including migrated Codex skill symlinks during install, syncs target-specific Codex/Claude user configuration, and uninstalls managed target-specific artifacts without running skill validation. |
...
Without `--target`, TTY installs use a color+Unicode Up/Down + Enter menu with `codex` selected by default; the menu states that skills always install to `$HOME/.agents/skills`, and non-TTY installs still default to `codex`. Install and uninstall completion output uses a grouped summary that shows only active effects for the selected target and omits skipped lines. During install, an existing skill symlink is adopted without `--force` only when it points to `skills/<skill>` in another worktree sharing the same Git common-dir. External symlinks, non-skill paths, and real directories keep the existing refusal or `--force` behavior.
