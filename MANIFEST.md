...
|---|---|
| `tools/` | 本地校验工具。 |
| `tools/fixtures/schema-sidecars/` | 随仓库提交的结构化索引回归样本，用于校验任务级产物路径、路由状态、状态转换和阶段必填字段。 |
| `tools/fixtures/runtime-sidecars/` | 随仓库提交的运行期结构化索引正负例，用于校验跨阶段轨迹连续性、授权门槛、最终交付前验证和 `reference_id` 一致性。 |
