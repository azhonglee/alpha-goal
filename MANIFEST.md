...
|---|---|
| `tools/` | 本地校验工具。 |
| `tools/fixtures/schema-sidecars/` | 随仓库提交的 schema sidecar 回归样本，用于校验任务级产物路径、路由状态、状态转换和阶段必填字段。 |
| `tools/fixtures/runtime-sidecars/` | 随仓库提交的运行期 schema sidecar 正负例，用于校验跨阶段轨迹连续性、授权门槛、最终交付前验证和 `reference_id` 一致性。 |
...
| `tools/validate_skills.ts` | 否 | 六技能闭环套件的轻量级规范校验器，覆盖引用可发现性、任务级产物布局、临时 `CODEX_HOME` 安装烟测、schema sidecar 样本 / 运行期样本、路由一致性、语义烟测、旧路径防护和样本契约检查。 |
...
| `.alpha-goal/YYYYMMDD-<slug>/schema/` | 可选机器可读 schema sidecar。 |
