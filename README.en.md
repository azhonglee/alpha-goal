...
- Persistent state: `goal-contract.md` is the default `alpha-goal` output; `technical_design.md` is created only by `references/technical-design-runbook.md` after Goal Contract Confirmation Gate selects `run technical design`; `checkpoint.md` is the required executor-to-verifier handoff, recording canonical artifact paths, per-repository execution context, checklist, and evidence.
- Recovery identity: use an explicit artifact path; otherwise accept only the sole active accepted candidate matching workspace, and stop on zero, multiple, terminal, or context-mismatched candidates.
...
- Strategy scenarios: `node tools/verify_strategy_scenarios.js` covers  stale evidence, post-PASS mutation, recovery ambiguity, concurrent execution, and stagnation routing.
