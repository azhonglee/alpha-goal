...
Runtime records use the user-level Alpha Goal state root: `${CODEX_HOME:-$HOME/.alphal-goal}/<workspace-slug>/`, where `<workspace-slug>` is the last directory name of the current session directory path. `alpha-goal` writes only `context.md`, `interview.md`, and `goal-contract.md` by default; `control-state/latest.md`, `run-profile.md`, `loop-state.md`, `memory.md`, `evidence.md`, and `verification.md` are conditional checkpoints created only for recovery, trigger handling, evidence handoff, or verification.
...
| `control-loop` | Execute or harden an authorized slice; `goal-contract.md` is required, while `run-profile.md`, `loop-state.md`, `memory.md`, and related files are conditional checkpoints. |
...
- Persistent state: `context.md`, `interview.md`, and `goal-contract.md` are the default `alpha-goal` outputs; `loop-state.md` records current state and the latest verification gap only when multi-turn recovery needs it, `iteration.md` records this run's facts only for handoff/recovery, and `memory.md` keeps only reusable confirmed facts, constraints, and strategy outcomes with evidence, confidence, and invalidation.
