...
```text
Trigger -> Preflight/Discovery -> Clarify Goal Contract -> Ask Design? -> Optional Technical Design -> Review -> Confirm
...
The installer copies the three public skills under `$HOME/.agents/skills/` and cleans same-repo links for merged old public skills. `global` and `claude` targets also create Claude skill symlinks under `$HOME/.claude/skills/` pointing at the shared copies. Without `--target`, interactive terminals use a color+Unicode arrow-key menu with `codex` highlighted by default, and non-interactive runs default to `codex`. Install and uninstall finish with a grouped summary that shows only active effects for the selected target and omits skipped lines; the install summary does not show `Result`, `Skills ... linked`, or `Install target`. Install migrates `skills/<skill>` symlinks from another worktree with the same Git common-dir into copied directories; external symlinks still require `--force` or are refused, same-name real directories are recopied, and ordinary files are refused. `--uninstall` removes managed configuration for the selected target; only `--uninstall --target global` removes managed skill copies under `$HOME/.agents/skills` and Claude skill symlinks under `$HOME/.claude/skills`. Uninstall does not follow configuration symlinks and does not remove external symlinks, mixed user configuration, or legacy Codex skills paths.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td align="left">Clarify intent, boundaries, and acceptance evidence, produce a Goal Contract for confirmation, and explicitly ask whether to enter design clarification before confirmation.</td>
    </tr>
    <tr>
...
- Persistent state: `goal-contract.md` is the default `alpha-goal` output; `technical_design.md` is created only after Design Choice Gate selects design clarification; `checkpoint.md` conditionally carries recovery and evidence handoff.
- Progressive disclosure: `alpha-goal` keeps Goal Contract clarification, design choice, and confirmation control flow in `SKILL.md`; Technical Design priorities, probes, and review checks live in `references/design-clarification-book.md`.
