...
The smoke test checks source-identical skill copies; semantic checkpoint transitions, JSON lock metadata, automatic unlock, stale-write rejection, abort/recovery, and compatibility commands from arbitrary CWD; config sync; adapter discoverability; hook upgrade; idempotence; failure preservation; concise output; conservative uninstall; rejected arguments; refused unmanaged targets; and ignored `CODEX_HOME`. It does not touch real user configuration.
...
compat_record="$(node "$lock_helper" acquire "$lock_checkpoint" executor:compat-abort 6 executor 7 executor)"
compat_token="$(node -p 'JSON.parse(process.argv[1]).token' "$compat_record")"
compat_pending="$(node -p 'JSON.parse(process.argv[1]).pendingPath' "$compat_record")"
test -n "$compat_pending"
node "$lock_helper" abort "$lock_checkpoint" "$compat_token" >/dev/null
assert_unlocked
compat_record="$(node "$lock_helper" acquire "$lock_checkpoint" executor:compat-release 6 executor 7 executor)"
compat_token="$(node -p 'JSON.parse(process.argv[1]).token' "$compat_record")"
node "$lock_helper" release "$lock_checkpoint" "$compat_token" >/dev/null
assert_unlocked
