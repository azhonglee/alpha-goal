...
When Custom Agent sync is selected, existing same-name regular files are replaced by the repository profiles whether or not they carry the managed marker; disable Custom Agent sync to preserve those files. Symlinks and non-regular same-name paths fail the whole install during preflight. Managed files are staged before activation and participate in the same rollback transaction; uninstall removes only files that still carry the managed marker.
