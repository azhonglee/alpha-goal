...
- It should identify `READ_ONLY_DISCOVERY`.
...
- With a concrete object, it should frame a `READ_ONLY_DISCOVERY` / `COMPARISON_ONLY` boundary.
...
- It should identify `READ_ONLY_DISCOVERY` or `DEBUG_GOAL` without mutation, depending on the requested claim.
...
- It should identify `DEBUG_GOAL`.
- FRAME should produce a Goal Contract with inline Spec.
