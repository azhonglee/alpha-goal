...
- 安装前运行源码中的 `tools/validate_skills.ts`。它会校验六技能结构、引用可发现性、安装脚本、配置模板、临时安装烟测、闭环语义、结构化伴随文件样本、运行期样本和样本契约。
...
- 当反馈重复、噪声大、影响范围广或风险高时，持久化控制律应包含反馈延迟、信号噪声、置信度、阻尼 / 防振荡和饱和 / 约束边界。
