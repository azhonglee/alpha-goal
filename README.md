...
一套建立在闭环控制方法论上的工作流技能集, 也是我在 next engineering paradigm 的一个探索。
...
| `goal-contract` | 形成可执行、可验证、可移交的 Goal Contract，并承接 Indicator Handoff | 目标、范围、验收、非目标或授权边界不清楚 |
| `system-model` | 建模 plant、state、observer、actuator、Controller Hierarchy、Disturbance Register 和 coupling | 系统边界、观测性、可控性、扰动或耦合会影响安全行动 |
| `control-loop` | 在已批准 Goal Contract 下执行有界迭代，采集反馈并记录 Adaptive Learning Record | 目标已明确，需要安全地执行、诊断、修复或加固 |
| `evidence-verify` | 判断证据是否支持完成、可合并、可发布或窄化声明 | 工作看似完成，需要独立检查证据和最终声明边界 |
| `decision-synthesis` | 通过 Synthesis Round 综合定性判断、机器证据、量化指标、冲突和用户裁决 | 多团队、多目标、高不确定性或复杂巨系统式决策 |
...
- 状态必须可恢复：长任务依靠 Closed-loop Ledger，而不是依赖聊天上下文。
