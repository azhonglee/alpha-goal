...
Alpha Goal 是面向 Goal Engineering 的最小持久闭环技能集。它帮助 agent 先发现事实再提问，从已接受 Goal Contract 和必要 checkpoint 恢复执行，在明确边界内行动，并且只在证据支持的范围内做最终声明。
...
运行态记录使用用户级 Alpha Goal state root：`${CODEX_HOME:-$HOME/.alphal-goal}/<workspace-slug>/`，其中 `<workspace-slug>` 是当前会话目录路径最后一个目录名。`alpha-goal` 默认只写已接受的 `goal-contract.md`，其中包含 discovery notes 和 interview ledger；`checkpoint.md` 是按恢复、触发、证据移交或验证需要创建的唯一任务级条件检查点；`control-state/latest.md` 只是全局恢复索引，不承载阶段内容。
