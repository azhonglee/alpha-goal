...
const SEMANTIC_CHECKS: Array<[string, string, string[]]> = [
  ...
  ["execution has hard safety gates", "skills/control-loop/SKILL.md", [
    ...
    "choose_highest_value_small_action_verifiable_now",
    ...
    "failing_outputs",
    "review_or_audit_or_loophole_finding",
...
function validateControlLoopStructure(root: string, errors: string[]): void {
  ...
  requireOrderedTerms("control-loop slice execution rules", markdownSection(text, "Slice Execution"), [
    ...
    "material_contradiction",
    "slice.kind == repair and not root_cause_confirmed",
    "return RETURN_TO_ALPHA_GOAL",
    "slice.kind in [implementation, hardening, repair]",
    "make_one_targeted_change_unless_coordinated_edits_required",
    "review_or_audit_or_loophole_finding",
    "standalone_final_judgment_without_goal_verify",
    "slice.kind not_in [implementation, hardening, repair, review_or_audit_or_loophole_finding]",
    "return BLOCKED",
    ...
    "evidence.is_fresh",
    "slice.complete only_if evidence.changes_or_confirms(goal.target_state)",
    "evidence.changes_or_confirms_target_state",
  ...
  requireOrderedTerms("control-loop routes", markdownSection(text, "Routes"), [
    ...
    "BLOCKED",
    "Stop/re-route",
    "changed_or_unclear(target or scope or authority or source_reference or acceptance_evidence or non_goal or decision_boundary or actuator_boundary or Trigger_Contract or Autonomy_level or claim_boundary)",
