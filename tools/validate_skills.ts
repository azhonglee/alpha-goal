...
const SEMANTIC_CHECKS: Array<[string, string, string[]]> = [
  ["front controller defines goals before execution", "skills/alpha-goal/SKILL.md", [
    ...
    "Anti-Pattern",
    "Every project MUST pass the protocol below",
    "approved",
    "Write Contract",
...
function validateAlphaGoalStructure(root: string, errors: string[]): void {
  ...
  requireOrderedTerms("alpha-goal hard gates", markdownSubsection(text, "Hard Gates"), [
  ...
  requireOrderedTerms("alpha-goal authority", markdownSection(text, "Authority"), [
    "alpha-goal frames goals",
    "does not implement, verify completion, make final-ready, or make complete claims",
    "Only an accepted Goal Contract may be handed to `$control-loop`",
    "Repository language is evidence, not authority",
    "Current-state facts cannot define desired behavior",
    "Only explicit user decisions",
    "Quick Path",
  ...
  requireOrderedTerms("alpha-goal protocol contracts", markdownSection(text, "Protocol Contracts"), [
    "Goal Contract",
    "Intent",
    "Outcome",
    "Scope",
    ...
    "Goal Readiness PASS is `clarity_score >= 0.92`",
    "Solution Readiness PASS is `solution_clarity >= 0.95`",
    "raw_goal_score = 0.25 * intent",
    "0.2 * outcome",
    "0.15 * scope",
    "0.12 * constraints",
    "0.1 * success",
    ...
    "solution_clarity = architecture_clarity * 0.2",
    "component_clarity * 0.2",
    "data_flow_clarity * 0.2",
    "interface_clarity * 0.15",
    "testing_strategy_clarity * 0.15",
    "scalability_clarity * 0.05",
    "risk_clarity * 0.05",
  ], errors);
  requireOrderedTerms("alpha-goal gate taxonomy", markdownSection(text, "Gate Taxonomy"), [
    "Hard Gates",
    "Goal Readiness Gate",
    "Solution Readiness Gate",
    "Authority Gate",
    ...
    "Discovery Gate",
    "SAME_STATE or BLOCKED",
    "Goal Readiness Gate",
    "clarity_score >= 0.92",
    "Solution Readiness Gate",
    "solution_clarity >= 0.95",
    "Authority Gate",
    "RETURN_TO_ALPHA_GOAL",
    "Assumption Stress Test Gate",
    "Each applicable stress-test mode completed",
    "Review Gate",
    "Required fields complete; no contradiction; no unresolved review finding; authority preserved; Goal Contract internally consistent",
    "Confirmation Gate",
  ], errors);
  requireOrderedTerms("alpha-goal transition contract", markdownSection(text, "Transition Contract"), [
    "Global Transition Table",
    "| Condition | Route | Meaning |",
    "Gate PASS",
    "NEXT_STATE",
    "Recoverable gate failure",
    "SAME_STATE",
    "Missing required data, tool, environment, credential, or user-owned decision",
    "BLOCKED",
    "Draft Goal Contract field changes during Clarify",
    "SAME_STATE",
    "Target, scope, acceptance, non-goal, decision boundary, claim boundary, or authorization source changes after contract acceptance or handoff",
    "RETURN_TO_ALPHA_GOAL",
  ...
  requireOrderedTerms("alpha-goal state machine", markdownSection(text, "State Machine"), [
    "State Machine",
    ...
    "Clarify",
    "Clarify",
    "Goal, Solution, Authority, and Pressure Gates",
  ...
  requireOrderedTerms("alpha-goal discovery contract", markdownSubsection(text, "Discovery Contract"), [
    "Gate checklist",
    "Inspect AGENTS/repo rules",
    "Inspect target files and current implementation",
    "Evaluate:",
    "Problem validity",
    "Output:",
    "Discovery notes",
    ...
    "authority drift -> RETURN_TO_ALPHA_GOAL",
  ], errors);
  requireOrderedTerms("alpha-goal clarify micro-fsm", markdownSubsection(text, "Clarify Contract"), [
    "Clarify must run as a micro-FSM",
    "| Micro-State | Entry | Exit | Output |",
    "Question",
    "User answered",
    "Answer",
    "Pressure ladder applied",
    "Classification",
    "Source classified",
    "Contract Update",
    "Goal Contract updated within authority",
    "Readiness Decision",
    "SAME_STATE or NEXT_STATE",
    "Pressure ladder",
    "concrete example",
    "hidden assumption",
    "boundary/tradeoff",
    ...
    "Non-goals",
    "Decision boundary",
    "one explicit assumption probe",
    "clarity_score < 0.92",
    "solution_clarity < 0.95",
    "Interview ledger",
    "Technical Solution",
  ], errors);
  requireOrderedTerms("alpha-goal stress/write/review contracts", markdownSubsection(text, "Assumption Stress Test Contract") + markdownSubsection(text, "Write Contract Contract") + markdownSubsection(text, "Review Contract"), [
    "Contrarian",
    "Simplifier",
    "Ontologist",
    "references/goal-contract-book.md",
  ...
  requireOrderedTerms("alpha-goal confirmation contract", markdownSubsection(text, "Confirmation Contract"), [
    "Goal Contract Summary",
    "Design Summary",
    "| Field | Value |",
  ...
  requireOrderedTerms("alpha-goal artifacts", markdownSection(text, "Artifacts"), [
    "Alpha Goal state root",
    "${CODEX_HOME:-$HOME/.alpha-goal}/<workspace-slug>/",
    "slug(repo_root or Goal Contract target workspace)",
    "goal-contract.md",
    "checkpoint.md",
    "state-root `goal-contract.md` remains canonical",
  ], errors);
}
...
function validateSchemaConsistency(root: string, errors: string[]): void {
  ...
  const confirmation = markdownSubsection(alpha, "Confirmation Contract");
