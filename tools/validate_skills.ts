...
const SEMANTIC_CHECKS: Array<[string, string, string[]]> = [
  ["front controller defines goals before execution", "skills/alpha-goal/SKILL.md", [
    ...
    "Goal Gates, Authority Gates, Context Gates, Repair Gate",
    "Q&A Loop",
...
function validateAlphaGoalStructure(root: string, errors: string[]): void {
  ...
  requireOrderedTerms("alpha-goal state machine", text, [
    "State Machine",
    "Execute phase states in order",
    "A phase may exit only when its exit conditions are satisfied",
    "If an exit condition fails, remain in the current phase",
  ...
  requireOrderedTerms("alpha-goal discovery state", markdownSection(text, "State: Discovery"), [
    "### Entry",
    "Goal initialized",
    "### Exit",
    "Discovery Gate",
  ...
  requireOrderedTerms("alpha-goal clarify state", markdownSection(text, "State: Clarify"), [
    "### Entry",
    ...
    "solution_clarity >= `0.95`",
    "Goal Gates, Authority Gates, Context Gates, Repair Gate",
    "Review, final user confirmation, accepted contract status, and the Before Handoff Checklist",
    "### Rules",
  ...
  requireOrderedTerms("alpha-goal assumption stress state", markdownSection(text, "State: Assumption Stress Test"), [
    "### Entry",
    "Clarify readiness passes",
    "### Exit",
    "Required modes completed",
    "### Rules",
    "Contrarian",
    "Simplifier",
    "Ontologist",
  ...
  requireOrderedTerms("alpha-goal contract/review states", markdownSection(text, "State: Write Contract") + markdownSection(text, "State: Review"), [
    "references/goal-contract-book.md",
    "Contract status: draft",
    "Self-review the Goal Contract",
    "Review",
  ], errors);
  requireOrderedTerms("alpha-goal confirmation state", markdownSection(text, "State: Confirmation"), [
    "### Entry",
    "Review Gate passes",
    "### Exit",
...
function validateSchemaConsistency(root: string, errors: string[]): void {
  ...
  const confirmation = markdownSection(alpha, "State: Confirmation");
