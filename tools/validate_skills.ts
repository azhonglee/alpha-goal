...
const SEMANTIC_CHECKS: Array<[string, string, string[]]> = [
  ["front controller frames/models/synthesizes/routes", "skills/alpha-goal/SKILL.md", [
    "Discover facts -> Pressure-test -> Confirm one decision", "Discovery interview", "minimum preflight", "never ask the user to summarize discoverable repository facts", "prompt-safe summary", "navigation evidence, not requirements or authority", "do not score, crystallize, or hand off", "repo language as evidence", "Existing patterns are compatibility signals", "treat the answer as a claim to reconcile", "[from-code][auto-confirmed]", "[from-research] external/current fact", "[from-user]", "auto-confirm only descriptive facts", "Current-state facts cannot define desired behavior", "current external best practices", "bounded fresh evidence", "residual ambiguity", "one decision variable", "exactly one `questions[]` item", "weakest readiness gate", "Pressure-test", "boundary scenario from inspected facts", "materially change execution", "non-goals", "decision boundaries", "mandatory gates", "Do not close on probable intent", "closure evidence", "Generic edit verbs", "read-only/probe authority does not imply mutation authority", "minimal reproducer", "Durable docs", "cannot upgrade", "Why not ask", "Why not execute", "Close the interview only", "Goal Contract", "Control Model", "Indicator Handoff", "user-owned decisions", "control-loop", "evidence-verify", "never creates", "Route Summary"
  ]],
  ["contract/model reference preserves pre-action discovery gates", "skills/alpha-goal/references/contract-and-model.md", [
    "Discovery Record:", "Trigger / skip reason", "Task / probable intent", "Prompt-safe context status", "Inspected facts/sources", "Current-state facts", "Desired-state evidence", "Inferences not yet confirmed", "Fact labels", "[from-code][auto-confirmed]", "[from-research] external/current fact", "[from-user]", "Docs/terminology ledger", "discoverable fact", "fact needing confirmation", "user-owned decision", "Ambiguity score", "Weakest readiness gate", "Pressure pass", "one high-leverage question", "Question type", "Closure state", "Closure evidence", "Closure summary", "route evidence", "current/external facts", "Current-state facts and existing patterns cannot become desired behavior", "user/evidence contradiction", "source-of-truth conflict", "source/authority", "Authorization source", "Goal Contract:", "Control Model:", "Non-goals", "Decision boundaries", "Acceptance evidence", "Claim boundary", "Authorization class", "Disturbance Register", "Controller Hierarchy", "Candidate Control Law"
  ]],
