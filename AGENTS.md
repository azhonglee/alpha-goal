...
本仓库维护 `alpha-goal` Codex 技能包。技能源码统一位于 `skills/`：核心入口是 `skills/alpha-goal/SKILL.md`；阶段技能位于 `skills/goal-iterate/`、`skills/goal-verify/`，`skills/goal-review/` 是显式辅助审查技能。各技能的详细规则放在本技能 `references/` 下，脚本放在本技能 `scripts/` 下。`templates/` 是可选用户配置模板，`scripts/install.sh` 负责软链接安装，`tools/validate_skillset.py` 用于本地布局校验。`README.md`、`INSTALL.md`、`MANIFEST.md` 应与这些路径和命令保持一致。
