...
这些原则是 agent 判断何时提问、何时行动、何时验证、何时回路由的决策规则。

- 先发现，再提问：先检查本地事实、文档、状态和既有契约；只把无法安全发现的决策交给用户。
- 把事实当证据，不当授权：当前代码事实只描述现状；代码和仓库模式没有用户、规格、issue 或已接受契约时，不能定义期望行为。
- 让目标边界控制执行：变更前先界定 outcome、scope、non-goals、acceptance evidence、决策 owner 和 claim boundary。
- 只建模影响控制的未知：只有当依赖、扰动或风险会改变下一步安全行动、验证边界或路由时，才把它纳入模型。
- 每轮只问一个用户拥有的决策：需要人判断时，只问一个会改变目标、范围、验收、风险、授权或最终声明的高杠杆问题。
- 用有界闭环执行：优先选择能被观察和比较的最小连贯行动或探针，而不是宽泛重构或猜测式清理。
- 独立验证声明：新鲜证据必须覆盖声明；执行成功、看起来合理或收窄措辞都不是证明。
- 按缺口路由：目标不清回到 `alpha-goal`，可修复的实现或证据缺口回到 `control-loop`，final/ready/safe/complete/repair 声明进入 `evidence-verify`。
