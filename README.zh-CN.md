...
| `system-model` | 建模被控对象、状态变量、观测器、执行器、控制器层级、扰动记录和耦合 | 系统边界、观测性、可控性、扰动或耦合会影响安全行动 |
...
| 状态记忆 | `.alpha-goal/YYYYMMDD-<slug>/control-state.md` 中的 闭环台账，记录参考输入、状态、误差、动作、反馈和下一路由 |
| 指标转译 | 把定性目标转成指标 / 代理、传感器、阈值和证据边界 |
| 自适应学习 | 当反馈推翻阈值、策略、路由或假设时，记录可复用但有边界的修正 |
| 扰动处理 | 通过 扰动记录 记录可能性、影响、传感器、控制措施和路由触发条件 |
| 分层协同控制 | 通过 控制器层级 识别全局 / 局部控制器、耦合变量、仲裁和升级条件 |
