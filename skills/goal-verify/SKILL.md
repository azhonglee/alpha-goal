...
1. Goal satisfaction: evidence covers the explicit Goal Contract, acceptance evidence, constraints, non-goals, any required run-profile checkpoints, and proposed claim.
...
Read canonical state-root Goal Contract, existing `loop-state.md`, `iteration.md`, `evidence.md`, `verification.md`, diff/artifacts, command outputs, tests, logs, reviewer/user feedback, and specs. Check but do not trust summaries.
...
- A verification used for `verification-triggered` recovery must bind to the checked Goal Contract and any loop-state or evidence artifacts it relies on; stale or cross-task bindings are gaps.
...
- Missing evidence for required run-profile checkpoints, named evaluators, evaluator route, trigger behavior, autonomy gate, required loop-state updates, required memory updates, or required defect/risk sweep is a gap for handoff or final claims.
...
- `NEXT_ITERATION`: goal satisfaction, evidence coverage, claim boundary, or defect/risk sweep does not prove the proposed final state. Choose next route by the Gap:
  - `control-loop` only for same-goal fixable evidence, test, edge, compatibility, cleanup, required loop-state, required memory, defect/risk, or verification-gap hardening.
...
The Gap must be specific enough for `$control-loop` to choose the next slice, and to set `loop-state.md` Next Slice when persistent recovery is needed, without reinterpreting the Goal Contract.
...
Persist `<Alpha Goal state root>/YYYYMMDD-<TaskName>/verification.md` for risky, final, review/audit, verification-triggered, or handoff claims. Use the base verdict for ordinary checks and add conditional sections only when they apply.
