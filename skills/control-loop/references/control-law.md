...
把表格作为内部控制律的面向用户展示。TUI 表格内容保持简短、便于扫读；较长推理、精确阈值和稳定性保护放入持久化控制律。只有在用户要求、持久化受阻，或高风险切片需要变更前逐项复核字段时，才在聊天中打印原始 `控制律:` 块。
...
当控制律影响恢复、审计或验证时，把完整结构持久化到迭代记录或闭环台账。它只是内部产物语法，不是默认 TUI 形态。结构化索引是机器可读的摘要与索引；除非结构化索引格式明确扩展，否则不要把结构化索引文件当成完整控制律。
