...
字段映射：`问题` 对应目标误差；`本轮动作` 对应控制变量和控制动作；`保持不变` 对应保持不变的变量和影响范围上限；`验收证据` 对应传感器和阈值；`主要风险` 对应信号噪声、阻尼 / 防振荡、影响范围上限或最强实质风险；`失败处理` 对应失败处理或停止 / 重新界定触发条件。表格内容保持简短，完整控制律放在持久化产物中。
...
调试时，修复前必须识别并记录根因。如果根因未确认，变更只能限于诊断探针、可逆诊断插桩，或明确用于验证假设且不改变目标修复范围的切片。
