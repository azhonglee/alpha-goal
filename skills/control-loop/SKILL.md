...
- `references/control-law.md`: 目标误差、控制变量、预期效果、传感器阈值、延迟 / 噪声 / 置信度、阻尼、饱和 / 约束边界和失败处理。任何变更或诊断探测切片前都要加载。
...
- 本切片的控制律：目标误差、控制变量、预期效果、传感器阈值、反馈延迟、信号噪声、置信度、阻尼 / 防振荡、饱和 / 约束边界和失败处理；
...
执行变更或诊断探测前，先准备完整控制律并写入迭代记录或台账，同时输出紧凑的 `执行检查` 表或等价的清晰计划段。默认不要在 TUI 打印原始 `控制律:` 块。只有持久化控制律包含目标误差、已批准控制变量、传感器阈值和失败处理时，才可行动；高风险或噪声大的循环还必须写明延迟、噪声、置信度、阻尼和饱和 / 约束边界。
...
字段对应关系：`问题`=目标误差；`本轮动作`=控制变量和动作；`保持不变`=不变变量和饱和 / 约束边界；`验收证据`=传感器和阈值；`主要风险`=信号噪声、阻尼 / 防振荡、饱和 / 约束边界或最强实质风险；`失败处理`=失败处理或停止 / 重新界定触发条件。表格保持简短，完整控制律放入持久化产物。
...
- 最新证据与证据类别；
- 验收变化与剩余误差；
- 控制律结果：预期效果、已观察反馈、阈值状态、反馈延迟、信号噪声、置信度、阻尼 / 防振荡、饱和 / 约束边界、失败处理或调整；
