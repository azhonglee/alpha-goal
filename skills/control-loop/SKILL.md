...
- 本切片的控制律：目标误差、控制变量、预期效果、传感器阈值、反馈延迟、信号噪声、置信度、阻尼 / 防振荡、影响范围上限和失败处理；
...
执行变更或诊断探测前，先准备完整控制律并写入迭代记录或台账。默认不要在 TUI 打印原始 `控制律:` 块；TUI 只显示 `执行检查` 表。只有持久化控制律包含目标误差、已批准控制变量、传感器阈值和失败处理时，才可行动；高风险或噪声大的循环还必须写明延迟、噪声、置信度、阻尼和影响范围上限。
...
字段对应关系：`问题`=目标误差；`本轮动作`=控制变量和动作；`保持不变`=不变变量和影响范围上限；`验收证据`=传感器和阈值；`主要风险`=信号噪声、阻尼 / 防振荡、影响范围上限或最强实质风险；`失败处理`=失败处理或停止 / 重新界定触发条件。表格保持简短，完整控制律放入持久化产物。
