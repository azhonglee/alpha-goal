...
不要因为普通实现任务存在少量未知项就建完整模型；只有未知项会改变改动边界、传感器、执行器、风险控制或最终声明时才使用本技能。
...
- 路由回 `goal-contract` 或 `alpha-goal` 前，必要时独立复核并更新模型；模型不能直接授权进入 `control-loop`。
...
如果已有台账，把最新路由、被控对象 / 当前状态假设、下一动作与已观察事实比较。路由回 `goal-contract` 或 `alpha-goal` 前，标记过期假设。
...
高影响或影响未知的扰动必须先具备传感器、控制措施和路由触发条件，才能把模型交给 `goal-contract` 固化执行边界。
...
默认先产出紧凑模型和 `模型摘要`。紧凑模型不是弱模型，仍要覆盖足以支撑契约的系统边界、传感器、执行器、扰动、耦合和残余不确定性。只有跨模块、高风险、需恢复/交接、用户要求或紧凑模型不足以支持目标契约时，才把完整控制模型写到 `.alpha-goal/YYYYMMDD-<slug>/system-model.md` 并更新闭环台账。只有用户要求、文件持久化受阻，或建模缺口需要显式复核时，才在聊天中打印完整模型。
...
- 推荐路由: goal-contract | evidence-verify | decision-synthesis | blocker
```

当模型足以写入或修订目标契约时，路由到 `goal-contract`。推荐路由不得把目标语义压缩成最小版本；应交接完整语义候选、待确认取舍，以及目标契约需要固化的语义。最小范围只用于后续执行切片，不用于裁剪用户意图。需要执行时，也先路由到 `goal-contract` 复核并交接；不要直接进入 `control-loop`。只有当下一动作是把证据与声明进行比较时，才路由到 `evidence-verify`。
