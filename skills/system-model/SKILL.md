...
为实质扰动创建清晰标记的扰动记录。当扰动的可能性、影响、传感器、控制措施或路由触发条件不明显时，加载 `references/disturbance-register.md`。
...
不要把实质扰动压缩成散文式风险列表。如果控制模型命名了实质扰动，却既没有输出扰动记录，也没有明确写明 `扰动记录: 无实质项`，该模型就是不完整的。
