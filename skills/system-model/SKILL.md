...
不要把实质性的多控制器关系压缩成普通协同说明。如果控制模型提到多个局部控制器会影响同一个全局目标，却既没有输出 `控制器层级:` 块，也没有明确写明 `控制器层级: 无实质项`，该模型就是不完整的。
...
不要把实质扰动压缩成普通风险列表。如果控制模型命名了实质扰动，却既没有输出扰动记录，也没有明确写明 `扰动记录: 无实质项`，该模型就是不完整的。
