---
name: evidence-verify
description: "判断新鲜证据是否满足活跃目标契约，并支持完成、正确性、安全性、可合并、可发布或窄化最终声明。用于最终比较器 / 误差边界决策，不用于实现。"
---

# 证据验证

本技能是闭环中的独立比较器，用来判断最终状态是否匹配参考状态、拟声明是否仍在证据范围内。

## 入口

当存在活跃或可恢复的已批准上下文，并且需要判断工作是否完成、正确、安全、可合并、可发布，或能否给出窄化最终声明时，使用本技能。

如果没有完成 / 就绪 / 正确性声明，不要把本技能用于普通建议性评审、安全扫描或只读审计。

正向结论必须有与风险相称、且能支撑声明语义的证据，覆盖：

- 参考状态、期望结果、范围内事项、排除范围 / 非目标、决策边界、约束和声明边界；
- 被引用的当前长期有效的规范 / 计划 / 模型；
- 迭代记录或等价 diff / 证据包；
- 每个实质 `control-loop` 切片的控制律结果；使用变更或诊断探测时，还要包含延迟 / 噪声 / 置信度和稳定性保护；
- 当定性目标或综合指标影响验收证据时，包含指标转译处理；
- 跨技能或跨轮次工作必须提供闭环台账，尤其是 `最新控制路由`；若禁止或无法写文件，则说明聊天态无法写入的原因；
- 当实质扰动影响路由、证据或风险时，包含扰动记录处理；
- 当反馈改变阈值、策略、路由或可复用假设时，包含自适应学习记录处理；
- 当声明涉及 bug 或根因修复时，包含调试回执；
- 最强实质风险和匹配的证据下限；
- 新鲜的最终目标仓库 / 产物状态和适用项目规则；
- 精确命令 / 探针 / 检查及结果，或明确阻塞项 / 替代证据；
- 对用户 / 评审 / 测试 / 运行态反馈的处理。

## 按需加载资源

- `references/verification-verdict-schema.md`: 正式结论字段语义。
- `references/completion-review-rubric.md`: 最终交付、可合并、可发布或生产敏感场景的证据下限。
- `references/claim-boundary-check.md`: 防止最终声明超出证据。
- `scripts/evidence-summary.ts`: 只读 diff / 状态证据。

## 流程

```text
映射验收 -> 检查产物 -> 检查声明边界 -> 判断结论 -> 路由
```

### 1. 映射验收

对每条验收或证据期望，识别新鲜最终状态证据、边界和状态：

- `covered`
- `partially covered`
- `not covered`
- `blocked`
- `not applicable`

证据必须匹配声明边界。低边界测试不能支撑更高边界的用户可见、服务、生产、租户、合规或安全声明。

### 2. 检查产物与风险

确认：

- 已批准上下文仍是当前状态，或已明确被取代；
- 目标契约、系统模型、长期有效的规范和计划语义一致，或矛盾已被路由；
- 台账中的参考状态、当前状态、最新控制路由、残余误差和最新路由决策与新鲜证据一致，或已明确被取代；
- 每个实质控制律都要写明目标误差、控制变量、预期效果、传感器阈值和反馈结果；高风险场景还要写明反馈延迟、信号噪声、置信度、阻尼 / 防振荡、饱和条件 / 约束边界和失败处理；
- 每个实质指标转译都应把定性目标映射到操作化定义、传感器、阈值 / 容差、测量时机和证据边界；
- 每个实质扰动记录条目都应包含传感器证据、控制措施、路由触发处理，或明确的残余缺口；
- 每条实质自适应学习记录都有证据、调整、复用条件、失效条件，且没有无证据支持的宽泛泛化；
- 迭代记录中的目标类型、控制切片、执行、反馈、学习和证据与最终 diff / 产物匹配；
- 已变更文件匹配目标且避开非目标；
- 变更证据来自隔离或已批准的编辑路径；
- `.worktrees/` 已被忽略或足够安全，且写入台账 / 证据产物前 `.alpha-goal/` 已被忽略；如果根目录 `.gitignore` 原本缺少 `.alpha-goal/`，初始化改动已包含在证据中；
- 检查在最后一次实质变更后运行；如果缺少检查，已有阻塞项 / 替代证据说明；
- 没有把重复、噪声大或延迟反馈过度声明为稳定最终证据；
- 失败输出已被理解，且不反驳最终声明；
- 反馈已处理、属于范围外，或已路由到其他位置。

bug / 根因修复必须先有 `ROOT_CAUSE_CONFIRMED`，才能声明修复完成。`NOT_REPRODUCED` 或 `BLOCKED` 不支持该声明。

### 3. 检查声明边界

比较：

```text
声明边界:
- 用户表述:
- 已实现边界:
- 已测试 / 已观察边界:
- 证据能支持的最高边界:
- 缺口:
- 允许的最终声明:
```

如果用户措辞是产品级，但证据只到辅助函数级，选择 `NEXT_ITERATION` 获取更宽证据，或选择 `NARROW_CLAIM_AND_FINAL` 并明确窄化措辞。

### 4. 判断结论

只返回一个结论：

- `PASS_TO_FINAL`: 证据覆盖验收和声明边界。
- `NARROW_CLAIM_AND_FINAL`: 局部目标已满足，但最终措辞必须窄于用户请求。
- `NEXT_ITERATION`: 方向有效，但仍需要实现、证据、加固或清理。
- `REFRAME`: 目标契约、系统模型、目标 / 范围、非目标、验收、既有工作关系或声明边界错误或不完整。
- `BLOCKED`: 缺少环境、数据、权限、凭证、工具，或用户自有风险 / 范围决策。

### 5. 输出

默认展示紧凑 Markdown 表格 `验证摘要`。紧凑输出只压缩展示，不降低证据新鲜度、声明边界、比较器或残余误差判断。只有可合并/可发布判断、高风险声明、证据存在争议、需要恢复/交接或用户要求时，才把完整验证结论写到 `.alpha-goal/YYYYMMDD-<slug>/verification-verdict.md`，并更新闭环台账产物登记。只有文件持久化受阻或最终声明需要显式复核时，才在聊天中打印完整结论。

紧凑版:

```text
验证结论:
- 结论:
- 证据覆盖:
- 声明边界:
- 判断:
- 缺口:
- 必需下一步:
- 允许的最终声明:
- 产物:
```

TUI 摘要:

```markdown
验证摘要

| 字段 | 内容 |
| --- | --- |
| 结论 | |
| 声明边界 | |
| 证据 | |
| 产物 | |
| 下一步 | |
```

完整版:

```text
验证结论:
- 结论:
- 验收证据矩阵:
- 契约 / 模型 / 产物复核:
- 声明边界:
- 风险 / 证据复核:
- 控制律复核:
- 指标转译复核:
- 自适应学习复核:
- 台账复核:
- 扰动复核:
- 已运行新鲜检查:
- 变更差异 / 范围复核:
- 反馈复核:
- 判断:
- 未解决缺口:
- 必需下一步:
- 允许的最终声明:
- 台账更新:
  - schema sidecar 路径:
```

路由:

- `PASS_TO_FINAL`: 最终答复可以在已验证边界内声明完成。
- `NARROW_CLAIM_AND_FINAL`: 最终答复必须说明窄化声明和剩余高边界缺口。
- `NEXT_ITERATION`: 返回 `control-loop`；不要声明完成。
- `REFRAME`: 返回 `goal-contract` 或 `system-model`；不要继续改动。
- `BLOCKED`: 报告阻塞项和最小缺失输入或权限。
