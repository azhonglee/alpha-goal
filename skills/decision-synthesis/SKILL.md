...
问题属于复杂系统，而不是单纯实现任务时，使用本技能。它在 `goal-contract` 或 `system-model` 继续前，先综合目标和决策边界。

如果只是代码结构复杂、但目标、风险接受和优先级都清楚，不使用本技能；直接进入 `goal-contract` 或已批准后的 `control-loop`。
...
默认先展示紧凑 Markdown 表格 `综合摘要`。只有跨轮次、需要恢复、用户要求，或用户自有决策需要复核完整取舍时，才把完整决策综合记录写到 `.alpha-goal/YYYYMMDD-<slug>/decision-synthesis.md` 并更新闭环台账。只有文件持久化受阻或用户需要复核完整取舍时，才在聊天中打印完整记录。
...
- 需要执行时，先路由到 `goal-contract` 固化目标契约；`decision-synthesis` 不能直接路由到 `control-loop`。
