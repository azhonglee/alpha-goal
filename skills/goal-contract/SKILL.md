...
资源：`references/ambiguity-scoring.md`、`references/goal-contract-schema.md`、`references/indicator-handoff.md`。
...
`inline` 展示 `契约摘要`；`persisted` / `audited` 写 `.alpha-goal/YYYYMMDD-<slug>/goal-contract.md` 和 `.alpha-goal/YYYYMMDD-<slug>/control-state.md`。
...
契约准备好后，请用户接受、拒绝或修改。接受后交接参考状态、完整语义取舍、模糊度结果、范围、非目标、证据下限、声明边界和执行器边界；否则停在 `user`。
