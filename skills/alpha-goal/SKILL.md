...
- 路由保持成比例：选择能降低实质不确定性的下一技能，不得收缩目标语义。
- 流程状态按任务风险分层记录；分层只改变展示、加载和持久化强度，不降低目标契约、授权、控制律、证据或最终声明闸门。需要持久化时写到 `.alpha-goal/`。仓库内首次写入前，确认 `.alpha-goal/` 已被忽略；如果根目录 `.gitignore` 缺少该条目，先加入 `.alpha-goal/`，再写台账产物。
- 目标清楚、单轮、低风险、无需恢复的任务可使用 `inline`：只在聊天中保留紧凑路由结论，不写台账文件。
...
```text
分类状态 -> 选择执行分层 -> 选择下一技能 -> 检查稳定性闸门 -> 按需持久化 -> 展示路由摘要
```
...
如果没有台账且任务可能跨多个技能、需要恢复、需要审计或风险较高，先确认 `.alpha-goal/` 已被忽略，再初始化 `.alpha-goal/YYYYMMDD-<slug>/control-state.md`。必要时先把 `.alpha-goal/` 加入根目录 `.gitignore`。

### 2. 选择执行分层

先选最轻的可安全分层。分层不是审批闸门，也不是降级标准；进入 `control-loop`、`evidence-verify` 或最终声明的前置条件保持不变。

| 分层 | 适用场景 | 产物 |
| --- | --- | --- |
| `inline` | 目标清楚、单轮、低风险、无需恢复或文件交接 | 聊天中的紧凑路由摘要 |
| `persisted` | 跨技能、多轮、需要恢复、存在实质扰动或用户要求流程产物 | `.alpha-goal/YYYYMMDD-<slug>/control-state.md` 和阶段产物 |
| `audited` | 高风险、外部副作用、需要审计轨迹、声明边界敏感或发布/生产相关 | `persisted` 加 schema sidecar 和一致性报告 |

只有 `persisted` 和 `audited` 默认写 `.alpha-goal/`。不要为了形式完整把 `inline` 任务升级。
...
| 已有活跃且批准的目标契约，需要变更或探测 | `goal-contract` 交接后接 `control-loop` | 先确认已批准契约仍是当前参考输入，再由契约交接执行 |
...
- 目标契约已被用户明确接受；若契约仍是草案或待确认状态，下一路由必须是 `user`，不能进入 `control-loop`；
- 进入 `control-loop` 的路由必须来自已批准的 `goal-contract` 交接；不得由 `alpha-goal`、`system-model` 或 `decision-synthesis` 直接进入；
...
- `persisted` 和 `audited` 台账记录上一误差信号，并说明选定技能如何降低误差；`inline` 在摘要中说明同一判断的路由和下一步；
- 最终声明由 `evidence-verify` 检查，而不是由执行阶段直接给出。
- 运行态产物使用 `references/artifact-layout.md` 定义的任务级布局；旧分类路径视为校验失败。
- `audited` 任务，或需要机器恢复、跨轮追踪、敏感交接安全的 `persisted` 任务，生成 `references/cybernetic-conformance.md` 一致性报告。

### 5. 持久化路由卡并展示摘要

`inline` 只展示紧凑摘要，不写完整路由卡。`persisted` 和 `audited` 把完整路由卡写入闭环台账。只有用户明确要求、持久化受阻，或路由风险高到必须逐字段复核时，才在 TUI 中打印完整路由卡。
