...
- Record Native Goal Sync target and result in checkpoint before handoff. After handoff, `verifier` supplies route semantics. The calling Agent owns native Goal lifecycle handling through the current runtime adapter. In Codex, `PASS_TO_FINAL` may support `update_goal(status=complete)` only when no required work remains and the verified target state is still current; if that update fails, report the sync failure and keep the native Goal unfinished. In evaluator-driven adapters without an update tool, follow the adapter-defined evaluator outcome. Other routes follow the verifier Route Contract.
