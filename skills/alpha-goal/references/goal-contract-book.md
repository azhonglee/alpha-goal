...
- `executor` and `verifier` recompute that digest on entry. A mismatch or missing digest invalidates acceptance and blocks use of the contract. The accepted authority digest identifies payload content; it does not prove ordering, historical uniqueness, or acceptance authenticity. A first handoff or new `checkpoint.jsonl` requires all current mandatory rows, while an earlier accepted payload lacking them may resume only through a log whose last valid record has the matching task and accepted digest; it gains no inferred authority. Legacy `checkpoint.md` remains on its compatibility protocol.
