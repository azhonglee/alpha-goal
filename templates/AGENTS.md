...
- Ensure `.worktrees/` is ignored before placing repository-local worktrees there. Default runtime evidence, reviews, scratch artifacts, and Closed-loop Ledger files go under `.alpha-goal/YYYYMMDD-<slug>/`; if `.alpha-goal/` is missing from the repo root `.gitignore`, add `.alpha-goal/` before writing those artifacts.
